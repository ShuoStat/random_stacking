...
random.predBLBin <- function(obj, newdata, type.BL = "response", ...) {
  predBLBin(obj, newdata, type.BL = type.BL, ifCVMods = TRUE)
}

random.predSLBin <- function(BLBin, alphas, type = c("response", "link"), ...) {
  ...
  for(i in seq_along(alphas)) {
    ...
    pred0 <- predSLBin(BLBin = BLBin, alphas = alphas[[i]], type = type)
...
predict.random.stk.bin <- function(obj, newdata, 
                                   type = c("response", "link"), 
                                   type.BL = "response", ...){
  
  BL <- random.predBLBin(obj = obj, newdata = newdata, type.BL = type.BL, ...) 
  SL <- random.predSLBin(BL, alphas = obj$alphas, type = type, ...)
  return(SL)
}
